...
extern fn import_random_bytes(buffer: [*]u8, len: usize) void;
...
pub const UUID = extern struct {
    ...
    pub fn seed() void {
        ...
        if (@import("builtin").target.os.tag == .freestanding and
            @import("builtin").cpu.arch == .wasm32)
        {
            import_random_bytes(&b, b.len);
        } else {
    ...
    pub fn v4() UUID {
        ...
        if (@import("builtin").target.os.tag == .freestanding and
            @import("builtin").cpu.arch == .wasm32)
        {
            import_random_bytes(&bin, bin.len);
        } else {
    ...
    pub fn v7() UUID {
        ...
        if (@import("builtin").target.os == .freestanding and
            @import("builtin").cpu.arch == .wasm32)
        {
            import_random_bytes(bin[8..], bin[8..].len);
        } else {
